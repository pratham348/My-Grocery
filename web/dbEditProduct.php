...
if(move_uploaded_file($temp_name, $target_path)) {
   ...
   $q = "UPDATE product SET category_id='$cate',product_name='$pname',Qty='$qty',product_amt='$amt',product_details='$detail',photo='$target_path' WHERE product_id='$proid' ";
   // $q = "INSERT INTO product(seller_id,category_id,product_name,Qty,product_amt,product_details,photo) VALUES('$seller','$cate','$pname','$qty','$amt','$detail','$target_path')";
    ...
    $data = mysqli_query($con,$q);
      ...
      // exit();
        if($data == true){
            header("location:MyProduct.php?message=Product Updated");
          }

        }
        else
        {
             header("location:EditProduct.php?error=Error While uploading Category on the Server");
             //print_r($errors);
        } 
